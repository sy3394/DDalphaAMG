...
OPT_VERSION_FLAGS = $(CFLAGS) $(LIMEFLAGS) ${FABULOUSFLAGS} $(H5FLAGS) ${LAPACKFLAGS} ${EFENCEFLAGS} -DPARAMOUTPUT -DTRACK_RES -DBASE_LOOP_COUNT=4 -DOPENMP -DPROFILING  -DTRACK_RES #-DHAVE_TM1p1 #-DDEBUG
...
$(TSTDIR)/%: $(LIB) $(TSTDIR)/%.c
	$(CC) $(CFLAGS) $(TEST_FLAGS) -o $@ $@.c -I$(INCDIR) -L$(LIBDIR) -lDDalphaAMG $(LIMELIB) $(LAPACKLIB) $(EFENCELIB) $(FABULOUSLIB) -lm 
