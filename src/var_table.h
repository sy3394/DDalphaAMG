...
#define SCAN_VAR( var_pt, kind, start_val, end_val, step_size, mult, name, nvec, l ) do{ \
    warning0("SCAN_VAR does not support threading, yet.\n");		\
    warning0("SCAN_VAR not tested yet.\n");				\
    kind *tmp_var = (kind*)(var_pt);					\
    kind signum = (start_val<end_val)?1:-1;				\
    vector_double v;							\
    vector_double_init(&v);						\
    double *norm_v, *tmp, tt0, tt1;					\
    vector_double x = (g.mixed_precision==2)?g.p_MP.dp.x:g.p.x;		\
    vector_double b = (g.mixed_precision==2)?g.p_MP.dp.b:g.p.b;		\
    tt0 = MPI_Wtime();							\
									\
    if ( g.vt.track_error ) {						\
      MALLOC( norm_v, double, nvec );					\
      MALLOC( tmp, double, nvec );					\
      vector_double_alloc( &v, _INNER, x.num_vect, l, no_threading );	\
	...
	if ( g.vt.re_setup ) {						\
	    ...
	    global_norm_double(tmp, &x, 0, l->inner_vector_size, l, no_threading ); \
            for (int i = 0; i < nvec; i++ ) g.vt.p_end->values[_CGNR_ERR] += tmp[i] / norm_v[i] /((double)g.vt.average_over*nvec); \
        ...
        if ( g.vt.track_error ) {					\
          vector_double_minus( &x, &x, &v, 0, l->inner_vector_size, l ); \
	  global_norm_double(tmp, &x, 0, l->inner_vector_size, l, no_threading ); \
          for (int i = 0; i < nvec; i++ ) g.vt.p_end->values[_SLV_ERR] += ( tmp[i] / norm_v[i] ) / ((double)g.vt.average_over*nvec); \
        }								\
      }									\
    }									\
    if ( g.vt.track_error ) {						\
      FREE( norm_v, double, nvec );					\
