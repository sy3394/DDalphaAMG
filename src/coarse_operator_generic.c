...
static void set_coarse_self_coupling_PRECISION_new( vector_PRECISION *buffer1, vector_PRECISION *buffer2, vector_PRECISION *V, const int n, level_struct *l );
static void set_coarse_neighbor_coupling_PRECISION_new( vector_PRECISION *buffer1, vector_PRECISION *buffer2, vector_PRECISION *V, const int mu, const int n, level_struct *l );
static void set_block_diagonal_PRECISION_new( vector_PRECISION *spin_0_1, vector_PRECISION *spin_2_3, vector_PRECISION *V, const int n, config_PRECISION block, level_struct *l );
static void coarse_spinwise_self_couplings_PRECISION_new( vector_PRECISION *eta1, vector_PRECISION *eta2, vector_PRECISION *phi, config_PRECISION clover, 
...
void coarse_operator_PRECISION_setup_new( vector_PRECISION *V, level_struct *l ) {
  ...
  void (*aggregate_self_coupling)()     = (l->depth==0)?d_plus_clover_aggregate_PRECISION_new:coarse_aggregate_self_couplings_PRECISION_new;
  ...
  for ( i=0; i<3; i++ ) {
    ...
    else vector_PRECISION_alloc( &buffer[i], V->type, num_loop, l, no_threading );
  ...
  for ( i=0; i<n; i++ ) {
    ...
    //    printf("set cor: %p %p %p\n", &buffer[0], &buffer[1],&buffer[2]);
    aggregate_self_coupling( &buffer[0], &buffer[1], &buffer[2], &(l->s_PRECISION), l );//printfv_PRECISION(&buffer[1]);
    vector_PRECISION_change_layout( V, V, _NVEC_OUTER, no_threading );
    vector_PRECISION_change_layout( &buffer[0], &buffer[0], _NVEC_OUTER, no_threading );
    vector_PRECISION_change_layout( &buffer[1], &buffer[1], _NVEC_OUTER, no_threading );//printfv_PRECISION(&buffer[1]);
    //       set l->next_level->op_PRECISION.clover)
    //vector_PRECISION_copy_new2( V, buffer[2], i, -1, l );
    //set_coarse_self_coupling_PRECISION_new( &buffer1, &buffer2, V, i, l );
    ...
    // odd_proj????
    //aggregate_block( &buffer1, &buffer2, V, l->s_PRECISION.op.odd_proj, l );
    aggregate_block( &buffer[0], &buffer[1], &buffer[2], l->s_PRECISION.op.odd_proj, l );
    //vector_PRECISION_change_layout( &buffer1, &buffer1, _NV_LV_SV, no_threading );
    //vector_PRECISION_change_layout( &buffer2, &buffer2, _NV_LV_SV, no_threading );
    vector_PRECISION_change_layout( &buffer[0], &buffer[0], _NVEC_OUTER, no_threading );
    vector_PRECISION_change_layout( &buffer[1], &buffer[1], _NVEC_OUTER, no_threading );
    //       set l->next_level->op_PRECISION.odd_proj
    //set_block_diagonal_PRECISION_new( &buffer1, &buffer2, V, i, l->next_level->op_PRECISION.odd_proj, l ); 
    set_block_diagonal_PRECISION_new( &buffer[0], &buffer[1], V, i, l->next_level->op_PRECISION.odd_proj, l ); 
    //vector_PRECISION_change_layout( &buffer1, &buffer1, _LV_SV_NV, no_threading );
    //vector_PRECISION_change_layout( &buffer2, &buffer2, _LV_SV_NV, no_threading );
  ...
  /*
  for ( j=0; j<block_size; j++ )
    printf("%g ",creal_PRECISION(l->next_level->op_PRECISION.odd_proj[j]));
  */
  t1 = MPI_Wtime();
...
static void set_coarse_self_coupling_PRECISION_new( vector_PRECISION *spin_0_1, vector_PRECISION *spin_2_3, 
  ...
  // diagonal coupling
  for ( k=0; k<=n; k++ ) { // for each vector in P
    k1 = (n*(n+1))/2+k; k2 = (n*(n+1))/2+k+(num_eig_vect*(num_eig_vect+1))/2;
  
    for ( j=0; j<num_aggregates; j++ ) { // for each aggregate
      spin_0_1_pt = spin_0_1->vector_buffer + (0*num_aggregates+j)*aggregate_size;//n->0
      spin_2_3_pt = spin_2_3->vector_buffer + (0*num_aggregates+j)*aggregate_size;//n->0
      interpolation_data = V->vector_buffer + k*V->size+j*aggregate_size;//head of j^th aggregate in k^th eigenvec:k*num_aggregates+j)*aggregate_size
  ...
  for ( k=0; k<num_eig_vect; k++ ) {
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      spin_0_1_pt = spin_0_1->vector_buffer + (0*num_aggregates+j)*aggregate_size;//n
      spin_2_3_pt = spin_2_3->vector_buffer + (0*num_aggregates+j)*aggregate_size;//n
      interpolation_data = V->vector_buffer + k*V->size+j*aggregate_size;
      clover_pt = clover + j*clover_site_size;
...
// n = num_eig_vect
static void set_block_diagonal_PRECISION_new( vector_PRECISION *spin_0_1, vector_PRECISION *spin_2_3, 
  ...
  //  printf("set_block_diagonal_PRECISION_\n");
  int i, j, k, m, k1, k2;
  int num_aggregates  = l->is_PRECISION.num_agg;
  int num_eig_vect    = l->next_level->num_parent_eig_vect;
  int aggregate_size  = l->num_inner_lattice_sites*l->num_parent_eig_vect*2/num_aggregates;
...
static void set_coarse_neighbor_coupling_PRECISION_new( vector_PRECISION *spin_0_1, vector_PRECISION *spin_2_3, 
  ...
  for ( k=0; k<num_eig_vect; k++ ) {
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_boundary_sites; i++ ) {
        spin_0_1_pt = spin_0_1->vector_buffer + 0*num_aggregates*aggregate_size + nlsv*index_dir[i1];//num_eig_vect
        interpolation_data = V->vector_buffer + k*V->size + nlsv*index_dir[i1]; i1++;//num_aggregates*aggregate_size 
        // A
        for ( m=0; m<offset; m++ )
    ...
    for ( j=0; j<num_aggregates; j++ ) {
      ...
      for ( i=0; i<aggregate_boundary_sites; i++ ) {
        spin_2_3_pt = spin_2_3->vector_buffer + 0*num_aggregates*aggregate_size + nlsv*index_dir[i1];//num_eig_vect
        interpolation_data = V->vector_buffer + k*V->size + nlsv*index_dir[i1]; i1++;//num_aggregates*aggregate_size 
        // B
        for ( m=0; m<offset; m++ )
...
void coarse_self_couplings_PRECISION_new( vector_PRECISION *eta, vector_PRECISION *phi,
  ...
  if (op->mu + op->mu_odd_shift != 0.0 || op->mu + op->mu_even_shift != 0.0 )
    coarse_add_anti_block_diagonal_PRECISION_new( &eta_pt, &phi_pt, op->tm_term+start*block_size, (end-start)*vector_size, l );//took out*op->pr_num_vect
#endif
...
void coarse_block_operator_PRECISION_new( vector_PRECISION *eta, vector_PRECISION *phi, int start, schwarz_PRECISION_struct *s, level_struct *l, struct Thread *threading ) {
  ...
  coarse_self_couplings_PRECISION_new( &leta, &lphi, &(s->op), (start/m), (start/m)+n, l);
...
void coarse_aggregate_self_couplings_PRECISION_new( vector_PRECISION *eta1, vector_PRECISION *eta2, vector_PRECISION *phi,
  ...
  vector_PRECISION_define_new( eta1, 0, 0, l->vector_size, l );
  vector_PRECISION_define_new( eta2, 0, 0, l->vector_size, l );  
  coarse_spinwise_self_couplings_PRECISION_new( eta1, eta2, phi, s->op.clover, l->inner_vector_size, l );
  ...
  for ( mu=0; mu<4; mu++ ) { // direction mu
    ...
    for ( i=0; i<length; i++ ) {
      ...
      coarse_spinwise_n_hopp_PRECISION_new( &eta1_pt, &eta2_pt, &phi_pt, D_pt, l );
      ...
      coarse_spinwise_n_daggered_hopp_PRECISION_new( &eta1_pt, &eta2_pt, &phi_pt, D_pt, l );
    }
  }
}

static void coarse_spinwise_self_couplings_PRECISION_new( vector_PRECISION *eta1, vector_PRECISION *eta2, vector_PRECISION *phi,
  ...
  while ( phi_pt < phi_end_pt ) {
    // A
    mvp_PRECISION_new( eta1_pt, clover_pt, phi_pt, num_eig_vect, nvec, nvec_eta1, nvec_phi );
    clover_pt += clover_step_size1; 
    phi_pt    += num_eig_vect*nvec_phi;
    eta1_pt   += num_eig_vect*nvec_eta1;
    // D
    mvp_PRECISION_new( eta2_pt, clover_pt, phi_pt, num_eig_vect, nvec, nvec_eta2, nvec_phi );
    clover_pt += clover_step_size1;
    phi_pt    -= num_eig_vect*nvec_phi; 
    eta2_pt   -= num_eig_vect*nvec_eta2;
    // C = -B*
    nmvh_PRECISION_new( eta1_pt, clover_pt, phi_pt, num_eig_vect, nvec, nvec_eta1, nvec_phi );
    phi_pt  += num_eig_vect*nvec_phi; 
    eta1_pt += num_eig_vect*nvec_eta1;
    // B
    mv_PRECISION_new( eta2_pt, clover_pt, phi_pt, num_eig_vect, nvec, nvec_eta2, nvec_phi );
...
void coarse_aggregate_neighbor_couplings_PRECISION_new( vector_PRECISION *eta1, vector_PRECISION *eta2, vector_PRECISION *phi,
  ...
  for ( i=0; i<length; i++ ) {
    ...
    eta1_pt.vector_buffer = eta1->vector_buffer + n*index1*nvec_eta1;
    eta2_pt.vector_buffer = eta2->vector_buffer + n*index1*nvec_eta2;
    coarse_spinwise_hopp_PRECISION_new( &eta1_pt, &eta2_pt, &phi_pt, D_pt, l );
  }
}

// 
void coarse_aggregate_block_diagonal_PRECISION_new( vector_PRECISION *eta1, vector_PRECISION *eta2, vector_PRECISION *phi,
  ...
  //  printf("coarse_aggregate_block_diagonal_:%d %d %d %d %d %d %d %d\n", nvec, nvec_phi, nvec_eta1, nvec_eta2, length, num_eig_vect, eta1->size, phi->size);
  if ( nvec_eta1 != nvec_eta2 || nvec_eta1 < nvec_phi )
  ...
  while ( phi_pt.vector_buffer < phi_end_pt.vector_buffer ) {
    // A
    //    printf("coarse_aggregate_block_diago %d %g", num_eig_vect,creal_PRECISION(*block));//*phi_end_pt.vector_buffer));
    mvp_PRECISION_new( eta1_pt.vector_buffer, block_pt, phi_pt.vector_buffer, num_eig_vect, nvec, nvec_eta1, nvec_phi );
    vector_PRECISION_define_new( &eta2_pt, _COMPLEX_PRECISION_ZERO, 0, num_eig_vect, l );
    ...
    // D
    //    printf("coarse_aggregate_block_diago D %d %g", num_eig_vect,creal_PRECISION(*block));//*phi_end_pt.vector_buffer));
    vector_PRECISION_define_new( &eta1_pt, _COMPLEX_PRECISION_ZERO, 0, num_eig_vect, l );
    mvp_PRECISION_new( eta2_pt.vector_buffer, block_pt, phi_pt.vector_buffer, num_eig_vect, nvec, nvec_eta2, nvec_phi );
    block_pt += block_step_size; 
    eta1_pt.vector_buffer += num_eig_vect*nvec_eta1; 
    eta2_pt.vector_buffer += num_eig_vect*nvec_eta2;
    phi_pt.vector_buffer  += num_eig_vect*nvec_phi;
...
void coarse_gamma5_PRECISION_new( vector_PRECISION *eta, vector_PRECISION *phi, int start, int end, level_struct *l ) {
  ...
  if( nvec_phi != nvec_eta )
    error0("coarse_gamma5_PRECISION_new: #vec eta should be #vec phi\n");
...
void apply_coarse_operator_PRECISION_new( vector_PRECISION *eta, vector_PRECISION *phi, operator_PRECISION_struct *op,
                                      level_struct *l, struct Thread *threading ) {
  PROF_PRECISION_START( _SC, threading );
  int start;
  int end;
  compute_core_start_end_custom(0, l->num_inner_lattice_sites, &start, &end, l, threading, 1);
  printf("apply_coarse_operator_PRECISION: %d %d %d %d\n",l->num_inner_lattice_sites,start, end, l->num_lattice_site_var);
  coarse_self_couplings_PRECISION_new( eta, phi, op, start, end, l);

  PROF_PRECISION_STOP( _SC, 1, threading );
  PROF_PRECISION_START( _NC, threading );

  coarse_hopping_term_PRECISION_new( eta, phi, op, _FULL_SYSTEM, l, threading );

  PROF_PRECISION_STOP( _NC, 1, threading );
}

/******************** NOT USED OR OUTDATED  *********************/

// did vectorization in the old version: used but has no effect: can eliminate???
void coarse_operator_PRECISION_set_couplings( operator_PRECISION_struct *op, level_struct *l,
                                              struct Thread *threading ) {

  printf0("coarse_operator_PRECISION_set_couplings: not impremented\n");
...
void apply_coarse_operator_dagger_PRECISION( vector_PRECISION *eta, vector_PRECISION *phi, operator_PRECISION_struct *op,
  ...
  coarse_gamma5_PRECISION_new( &(l->vbuf_PRECISION[3]), phi, threading->start_index[l->depth], threading->end_index[l->depth], l );
  apply_coarse_operator_PRECISION_new( eta, &(l->vbuf_PRECISION[3]), op, l, threading );
  coarse_gamma5_PRECISION_new( eta, eta, threading->start_index[l->depth], threading->end_index[l->depth], l );
}
*/
/**************** TEST ROUTINES *****************************************************************************************/
...
void coarse_tau1_gamma5_PRECISION( vector_PRECISION *eta, vector_PRECISION *phi, int start, int end, level_struct *l ) {
  ...
  if ( g.n_flavours == 2 ) {
...
#endif
    {
      ...
      coarse_gamma5_PRECISION_new( eta, phi, start, end, l );
    }
}

void coarse_operator_PRECISION_test_routine_new( level_struct *l, struct Thread *threading ) {

  if ( !l->idle ) {
    int ivs = l->inner_vector_size, civs = l->next_level->inner_vector_size;
    int n_vect = g.num_rhs_vect, j, jj;
    PRECISION diff = 0, diff1[n_vect], diff2[n_vect];
...
/*    {
    ...
    if ( !l->next_level->idle )
      vector_PRECISION_define_random_new( &vc[0], 0, civs, l->next_level );
    vector_PRECISION_distribute_new( &vc[1], &vc[0], l->next_level );
    vector_PRECISION_gather_new( &vc[2], &vc[1], l->next_level );
    if ( !l->next_level->idle ) {
      vector_PRECISION_minus_new( &vc[1], &vc[0], &vc[2], 0, civs, l->next_level );
      global_norm_PRECISION_new( diff1, &vc[1], 0, civs, l->next_level, no_threading );
      global_norm_PRECISION_new( diff2, &vc[0], 0, civs, l->next_level, no_threading );
    }
    ...
    if ( !l->next_level->idle )
      vector_PRECISION_define_random_new( &vc[0], 0, civs, l->next_level );
    interpolate3_PRECISION_new( &vp[0], &vc[0], l, no_threading );
    restrict_PRECISION_new( &vc[1], &vp[0], l, no_threading );
    if ( !l->next_level->idle ) {
      vector_PRECISION_minus_new( &vc[2], &vc[0], &vc[1], 0, civs, l->next_level );
      global_norm_PRECISION_new( diff1, &vc[2], 0, civs, l->next_level, no_threading );
      global_norm_PRECISION_new( diff2, &vc[0], 0, civs, l->next_level, no_threading );
      for(int i=0; i<n_vect; i++)
    ...
    if(threading->n_core>1) {
      interpolate3_PRECISION_new( &vp[0], &vc[0], l, threading );
      restrict_PRECISION_new( &vc[1], &vp[0], l, threading );
      START_LOCKED_MASTER(threading)
      if ( !l->next_level->idle ) {
        vector_PRECISION_minus_new( &vc[2], &vc[0], &vc[1], 0, civs, l->next_level );
        global_norm_PRECISION_new( diff1, &vc[2], 0, civs, l->next_level, no_threading );
    ...
    START_LOCKED_MASTER(threading)
    if (l->depth==0) 
      gamma5_PRECISION_new( &vp[1], &vp[0], l, no_threading );
    else
      coarse_gamma5_PRECISION_new( &vp[1], &vp[0], 0, ivs, l );
    restrict_PRECISION_new( &vc[1], &vp[1], l, no_threading );
    coarse_gamma5_PRECISION_new( &vc[2], &vc[1], 0, civs, l->next_level );
    if ( !l->next_level->idle ) {
      vector_PRECISION_minus_new( &vc[1], &vc[0], &vc[2], 0, civs, l->next_level );
      global_norm_PRECISION_new( diff1, &vc[1], 0, civs, l->next_level, no_threading );
      global_norm_PRECISION_new( diff2, &vc[0], 0, civs, l->next_level, no_threading );
...
/*#ifdef HAVE_TM1p1
    ...
    START_LOCKED_MASTER(threading)
    vector_PRECISION_define_random_new( &vp[1], 0, ivs, l );
    vector_PRECISION_change_layout(&vp[2],&vp[1], _NVEC_OUTER, no_threading );//_NV_LV_SV==0
    vector_PRECISION_change_layout(&vp[2],&vp[2], _NVEC_INNER, no_threading );
    vector_PRECISION_minus_new( &vp[2], &vp[2], &vp[1], 0, civs, l->next_level );
    global_norm_PRECISION_new( diff1, &vp[2], 0, civs, l, no_threading );
    global_norm_PRECISION_new( diff2, &vp[1], 0, civs, l, no_threading );
    for(int i=0; i<n_vect; i++)
    ...
    START_LOCKED_MASTER(threading)
      vector_PRECISION_define_random_new( &vc[0], 0, civs, l->next_level );
    interpolate3_PRECISION_new( &vp[0], &vc[0], l, no_threading );
    vector_PRECISION_define_new( &vp[1], 0, 0, ivs, l );//printfv_PRECISION(&vp[1]);
    if (l->depth==0) 
      add_diagonal_PRECISION_new( &vp[1], &vp[0], l->s_PRECISION.op.odd_proj, ivs );
    else
      coarse_add_block_diagonal_PRECISION_new( &vp[1], &vp[0], l->s_PRECISION.op.odd_proj, ivs, l );
    restrict_PRECISION_new( &vc[1], &vp[1], l, no_threading );
      
    VECTOR_LOOP(j, n_vect, jj, factor[j+jj] = -1.0;)

    vector_PRECISION_scale_new( &vc[1], &vc[1], factor, 0, 0, civs, l->next_level );
    coarse_add_block_diagonal_PRECISION_new( &vc[1], &vc[0], l->next_level->s_PRECISION.op.odd_proj, civs, l->next_level );
    ...
    if (g.mu + g.mu_odd_shift != 0.0 || g.mu + g.mu_even_shift != 0.0 ) {
      vector_PRECISION_define_new( &vp[1], 0, 0, ivs, l );
      if (l->depth==0) 
        add_diagonal_PRECISION_new( &vp[1], &vp[0], l->s_PRECISION.op.tm_term, ivs );
      else
        coarse_add_anti_block_diagonal_PRECISION_new( &vp[1], &vp[0], l->s_PRECISION.op.tm_term, ivs, l );
      restrict_PRECISION_new( &vc[1], &vp[1], l, no_threading );
      
      VECTOR_LOOP(j, n_vect, jj, factor[j+jj] = -g.mu_factor[l->next_level->depth]/g.mu_factor[l->depth];)

      vector_PRECISION_scale_new( &vc[1], &vc[1], factor, 0, 0, civs, l->next_level );
      coarse_add_anti_block_diagonal_PRECISION_new( &vc[1], &vc[0], l->next_level->s_PRECISION.op.tm_term, civs, l->next_level );
      global_norm_PRECISION_new( diff1, &vc[1], 0, civs, l->next_level, no_threading );
      global_norm_PRECISION_new( diff2, &vc[0], 0, civs, l->next_level, no_threading );
      for(int i=0; i<n_vect; i++)
...
/*#ifdef HAVE_TM1p1
    ...
    if ( l->level > 0 ) {
      START_LOCKED_MASTER(threading)
      interpolate3_PRECISION_new( &vp[0], &vc[0], l, no_threading );
      printf("eres %d %d\n",vc[0].num_vect_now,vp[0].num_vect_now);
      ...
      if (g.mu + g.mu_odd_shift != 0.0 || g.mu + g.mu_even_shift != 0.0 )
	    ...
	    //	    printf("coarse_op test tM\n");
          vector_PRECISION_scale_new( &vp[2], &vp[0], factor, 0, 0, ivs, l );
          if(l->depth == 0)
            add_diagonal_PRECISION_new( &vp[1], &vp[2], l->p_PRECISION.op->tm_term, ivs );//tm==0!!!!!
          else
            coarse_add_anti_block_diagonal_PRECISION_new( &vp[1], &vp[2], l->p_PRECISION.op->tm_term, ivs, l );
        }
#endif
      restrict_PRECISION_new( &vc[1], &vp[1], l, no_threading );
      printf("eres %d %d\n",vc[1].num_vect_now,vp[1].num_vect_now);
      if ( !l->next_level->idle ) {
        if ( l->level==1 && g.odd_even )
          coarse_odd_even_PRECISION_test_new( &vc[2], &vc[0], l->next_level, no_threading );
        else
	...
	for(int i=0;i<civs*n_vect;i++)printf0("%d %d %d %13g %g %g\n",i/n_vect/l->next_level->num_lattice_site_var,(i/n_vect)%l->next_level->num_lattice_site_var,i%n_vect,creal_PRECISION(vc[1].vector_buffer[i]-vc[2].vector_buffer[i]),creal_PRECISION(vc[1].vector_buffer[i]),creal_PRECISION(vc[2].vector_buffer[i]));
        ...
        vector_PRECISION_minus_new( &vc[2], &vc[1], &vc[2], 0, civs, l->next_level );
        global_norm_PRECISION_new( diff1, &vc[2], 0, civs, l->next_level, no_threading );
        global_norm_PRECISION_new( diff2, &vc[1], 0, civs, l->next_level, no_threading );
	//	printf("%g diff2 %g\n",diff1[0],diff2[0]);
      ...
      if(threading->n_core>1) {
        if ( !l->next_level->idle ) {
          if ( l->level==1 && g.odd_even )
            coarse_odd_even_PRECISION_test_new( &vc[2], &vc[0], l->next_level, threading );
          else
    ...
    if ( l->level > 0 && l->depth > 0 && g.method == 3 && g.odd_even ) {
      vector_PRECISION_define_random_new( &vp[0], 0, ivs, l );
      block_to_oddeven_PRECISION_new( &vp[3], &vp[0], l, no_threading );
      coarse_diag_ee_PRECISION_new( &vp[2], &vp[3], &(l->oe_op_PRECISION), l, no_threading );
      coarse_diag_oo_PRECISION_new( &vp[2], &vp[3], &(l->oe_op_PRECISION), l, no_threading );
      coarse_hopping_term_PRECISION_new( &vp[2], &vp[3], &(l->oe_op_PRECISION), _FULL_SYSTEM, l, no_threading );
      oddeven_to_block_PRECISION_new( &vp[3], &vp[2], l, no_threading );
      apply_operator_PRECISION( &vp[1], &vp[0], &(l->p_PRECISION), l, no_threading );
      vector_PRECISION_minus_new( &vp[3], &vp[3], &vp[1], 0, ivs, l );
      global_norm_PRECISION_new( diff1, &vp[3], 0, ivs, l, no_threading );
      global_norm_PRECISION_new( diff2, &vp[1], 0, ivs, l, no_threading );
      for(int i=0; i<n_vect; i++)
        test0_PRECISION("depth: %d, correctness of odd even layout (smoother): %le\n", l->depth, diff1[i]/diff2[i] );
     
      block_to_oddeven_PRECISION_new( &vp[3], &vp[0], l, no_threading );
      coarse_odd_even_PRECISION_test_new( &vp[2], &vp[3], l, no_threading );
      oddeven_to_block_PRECISION_new( &vp[3], &vp[2], l, no_threading );
      apply_operator_PRECISION( &vp[1], &vp[0], &(l->p_PRECISION), l, no_threading );
      vector_PRECISION_minus_new( &vp[3], &vp[3], &vp[1], 0, ivs, l );
      global_norm_PRECISION_new( diff1, &vp[3], 0, ivs, l, no_threading );
   ...
   END_LOCKED_MASTER(threading)
    ...
    if ( l->next_level->level > 0  && !l->next_level->idle ) {
      schwarz_PRECISION_mvm_testfun_new( &(l->next_level->s_PRECISION), l->next_level, threading );
    }

    if ( l->next_level->level > 0 && !l->next_level->idle )
      coarse_operator_PRECISION_test_routine_new( l->next_level, threading );
