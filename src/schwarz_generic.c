...
void smoother_PRECISION_def( level_struct *l ) { // called only at the top level

  // alloc and setup Schwarz struct l->s_PRECISION
  //   Dirac operator and the clover term are copied from g.op_double into the Schwarz l->s_PRECISION
  if ( g.method >= 0 ) {
  ...
  if ( g.method == 4 ) {//FGMRES + GMRES (no AMG)
...
void schwarz_PRECISION_alloc( schwarz_PRECISION_struct *s, level_struct *l ) {
  ...
  */
  ...
  for ( i=0; i<3; i++)
    vector_PRECISION_alloc( &(s->local_minres_buffer[i]), _SCHWARZ, nvec, l, no_threading );
...
void schwarz_layout_PRECISION_define( schwarz_PRECISION_struct *s, level_struct *l ) {
  ...
  // for each aggregate: d0,...,a0 are coordinats for the aggregates in the local lattice
  for ( d0=0; d0<agg_split[T]; d0++ )
    for ( c0=0; c0<agg_split[Z]; c0++ )
      for ( b0=0; b0<agg_split[Y]; b0++ )
        for ( a0=0; a0<agg_split[X]; a0++ ) {
          // for each block in the aggregate: d1,...,a1 are coordinates for the blocks in the local lattice
          for ( d1=d0*block_split[T]; d1<(d0+1)*block_split[T]; d1++ )
            for ( c1=c0*block_split[Z]; c1<(c0+1)*block_split[Z]; c1++ )
              for ( b1=b0*block_split[Y]; b1<(b0+1)*block_split[Y]; b1++ )
                for ( a1=a0*block_split[X]; a1<(a0+1)*block_split[X]; a1++ ) {
                  ...
                  s->block[j].start = i;   // Schwarz index on the first site in the j^th block
                  s->block[j].no_comm = 1; // 1 if no communication is necessary, i.e., the block is not on the boundary; 0 otherwise
                  if ( s->num_colors == 1 ) {
                    s->block[j].color = 0; // color index of the given block
		    ...
		    // change the communication flag if a given block is on the boundary
                    for ( mu=0; mu<4; mu++ ) {
                    ...
                    if ( s->num_colors == 2 ) {
                      // identify the block's location in relation to the boundaries
		      ...
		      // categorize blocks according to its location in relation to the boundaries (red:0,1,2,3; black:4,5,6,7)
                      if ( count_inner == 4 ) {       // inner block: (red,black) = (0,4)
                        index = 4*s->block[j].color;
                      } else if ( count_minus == 0 ) {// not adjacent to neg boundaries: (red,black) = (1,7)
                        if ( s->block[j].color == 0 ) index = 1;
                        else index = 7;
                      } else if ( count_plus == 0 ) { // not adjacent to pos boundaries: (red,black) = (3,5)
                        if ( s->block[j].color == 0 ) index = 3;
                        else index = 5;
                      } else {                        // blocks on the corner of neg and pos boundaries: (red,black) = (2,6)
                        index = 2 + 4*s->block[j].color;
                      }
		      // record which block falls into which category
...
#ifdef HAVE_TM1p1
  epsbar_term_PRECISION_setup( (PRECISION) (g.epsbar_factor[l->depth]*op_in->epsbar), (PRECISION) (g.epsbar_factor[l->depth]*op_in->epsbar_ig5_even_shift),
			       (PRECISION) (g.epsbar_factor[l->depth]*op_in->epsbar_ig5_odd_shift), &(s->op), l, no_threading );
#endif
...
/*#ifdef HAVE_TM1p1_old
  if( g.n_flavours == 2 ) {
    ...
    for ( i=bbl[2*mu]; i<bbl[2*mu+1]; i+=2 ) {
      ...
      phi_pt = phi->vector_buffer + 12*neighbor_index*nvec_phi;
      eta_pt = eta->vector_buffer + 12*index*nvec_eta;
      prp_Z_PRECISION( buf1, phi_pt, nvec, nvec, nvec_phi );
      mvm_PRECISION( buf2, D_pt, buf1, nvec, nvec, nvec );
      mvm_PRECISION( buf2+3*nvec, D_pt, buf1+3*nvec, nvec, nvec, nvec );
      pbp_su3_Z_PRECISION( buf2, eta_pt, nvec, nvec, nvec_eta );
    }
    // minus mu direction
    for ( i=bbl[2*mu+1]; i<bbl[2*mu+2]; i+=2 ) {
      index = s->block[k].bt[i];
      ...
      phi_pt = phi->vector_buffer + 12*neighbor_index*nvec_phi;
      eta_pt = eta->vector_buffer + 12*index*nvec_eta;
      prn_Z_PRECISION( buf1, phi_pt, nvec, nvec, nvec_phi );
      mvmh_PRECISION( buf2, D_pt, buf1, nvec, nvec, nvec );
      mvmh_PRECISION( buf2+3*nvec, D_pt, buf1+3*nvec, nvec, nvec, nvec );
      pbn_su3_Z_PRECISION( buf2, eta_pt, nvec, nvec, nvec_eta );
    }
    ...
    for ( i=bbl[2*mu]; i<bbl[2*mu+1]; i+=2 ) {
      ...
      phi_pt = phi->vector_buffer + 12*neighbor_index*nvec_phi;
      eta_pt = eta->vector_buffer + 12*index*nvec_eta;
      prp_Y_PRECISION( buf1, phi_pt, nvec, nvec, nvec_phi );
      mvm_PRECISION( buf2, D_pt, buf1, nvec, nvec, nvec );
      mvm_PRECISION( buf2+3*nvec, D_pt, buf1+3*nvec, nvec, nvec, nvec );
      pbp_su3_Y_PRECISION( buf2, eta_pt, nvec, nvec, nvec_eta );
    }
    // minus mu direction
    for ( i=bbl[2*mu+1]; i<bbl[2*mu+2]; i+=2 ) {
      index = s->block[k].bt[i];
      ...
      phi_pt = phi->vector_buffer + 12*neighbor_index*nvec_phi;
      eta_pt = eta->vector_buffer + 12*index*nvec_eta;
      prn_Y_PRECISION( buf1, phi_pt, nvec, nvec, nvec_phi );
      mvmh_PRECISION( buf2, D_pt, buf1, nvec, nvec, nvec );
      mvmh_PRECISION( buf2+3*nvec, D_pt, buf1+3*nvec, nvec, nvec, nvec );
      pbn_su3_Y_PRECISION( buf2, eta_pt, nvec, nvec, nvec_eta );
    }
    ...
    for ( i=bbl[2*mu]; i<bbl[2*mu+1]; i+=2 ) {
      ...
      phi_pt = phi->vector_buffer + 12*neighbor_index*nvec_phi;
      eta_pt = eta->vector_buffer + 12*index*nvec_eta;
      prp_X_PRECISION( buf1, phi_pt, nvec, nvec, nvec_phi );
      mvm_PRECISION( buf2, D_pt, buf1, nvec, nvec, nvec );
      mvm_PRECISION( buf2+3*nvec, D_pt, buf1+3*nvec, nvec, nvec, nvec );
      pbp_su3_X_PRECISION( buf2, eta_pt, nvec, nvec, nvec_eta );
    }
    // minus mu direction
    for ( i=bbl[2*mu+1]; i<bbl[2*mu+2]; i+=2 ) {
      index = s->block[k].bt[i];
      ...
      phi_pt = phi->vector_buffer + 12*neighbor_index*nvec_phi;
      eta_pt = eta->vector_buffer + 12*index*nvec_eta;
      prn_X_PRECISION( buf1, phi_pt, nvec, nvec, nvec_phi );
      mvmh_PRECISION( buf2, D_pt, buf1, nvec, nvec, nvec );
      mvmh_PRECISION( buf2+3*nvec, D_pt, buf1+3*nvec, nvec, nvec, nvec );
      pbn_su3_X_PRECISION( buf2, eta_pt, nvec, nvec, nvec_eta );
    }  
/*#ifdef HAVE_TM1p1_old
  }
#endif*/
...
// eta += coarse_block_boundary_op*phi
...
void schwarz_PRECISION( vector_PRECISION *phi, vector_PRECISION *D_phi, vector_PRECISION *eta, const int cycles, int res,
  ...
  for ( i=nb_thread_start; i<nb_thread_end; i++ ) {
    if ( l->relax_fac != 1.0 )
      vector_PRECISION_scale( phi, x, relax_factor, 0, s->block[i].start*l->num_lattice_site_var, s->block[i].start*l->num_lattice_site_var+s->block_vector_size, l );
    else
    ...
    for ( i=nb_thread_start; i<nb_thread_end; i++ ) {
      if ( 0 == s->block[i].color && s->block[i].no_comm ) {
        ...
        vector_PRECISION_minus( D_phi, eta, r, s->block[i].start*l->num_lattice_site_var,
                                s->block[i].start*l->num_lattice_site_var+s->block_vector_size, l );
        if ( l->relax_fac != 1.0 ) {
          vector_PRECISION_scale( D_phi, D_phi, relax_factor, 0, s->block[i].start*l->num_lattice_site_var,
                                  s->block[i].start*l->num_lattice_site_var+s->block_vector_size, l );
        }
      }
  ...
  if( ! (g.epsbar != 0 || g.epsbar_ig5_odd_shift != 0 || g.epsbar_ig5_odd_shift != 0))
    nvecsf *=  g.n_flavours;
#endif
  PRECISION r_norm[nvecsf];
  global_norm_PRECISION( r_norm, r, 0, l->inner_vector_size, l, no_threading );
...
void red_black_schwarz_PRECISION( vector_PRECISION *phi, vector_PRECISION *D_phi, vector_PRECISION *eta, const int cycles, int res,
  ...
  for ( k=0; k<cycles; k++ ) {
    for ( step=0; step<8; step++ ) { // for each category of blocks partitioned based on loc relative to the boundaries
      for ( i=block_thread_start[step]; i<block_thread_end[step]; i++ ) { // for each block in the given category
        int index = s->block_list[step][i]; // which block
        START_MASTER(threading)
        PROF_PRECISION_START( _SM3 );
        END_MASTER(threading)
	// update local resiudal coming from the boundary
        if ( res == _RES ) {
          if ( k==0 && init_res == _RES ) {
            ...
            vector_PRECISION_minus( r, eta, Dphi, s->block[index].start*l->num_lattice_site_var,
				    s->block[index].start*l->num_lattice_site_var+s->block_vector_size, l ); // r = eta - Dphi
          } else {
            n_boundary_op( r, latest_iter, index, s, l ); // r <- r - boundary_op*latest_iter
          }
        }
        START_MASTER(threading)
        PROF_PRECISION_STOP( _SM3, 1 );
  ...
  // copy phi = x possibly with a factor
  if ( l->relax_fac != 1.0 )
    vector_PRECISION_scale( phi, x, relax_factor, 0, start, end, l );
  else
    vector_PRECISION_copy( phi, x, start, end, l );
  
  // calculate D * phi from r to extend Krylov space by one
  if ( D_phi != NULL ) {
    // compute local contribution, (D*phi)_b = (eta - r)_b
    for ( step=4; step<8; step++ ) { // local r on black blocks are up-to-date 
      for ( i=block_thread_start[step]; i<block_thread_end[step]; i++ ) {
        ...
        if ( l->relax_fac != 1.0 ) {
          vector_PRECISION_scale( D_phi, D_phi, relax_factor, 0, s->block[index].start*l->num_lattice_site_var,
                                  s->block[index].start*l->num_lattice_site_var+s->block_vector_size, l );
        }
      }
    }
    // compute boundary contribution to (D*phi)_b
    for ( step=0; step<4; step++ ) { // local r on red blocks needs to be updated
      for ( i=block_thread_start[step]; i<block_thread_end[step]; i++ ) {
        int index = s->block_list[step][i];
        START_MASTER(threading)
        PROF_PRECISION_START( _SM3 );
        END_MASTER(threading)
	// complete computation of r by taking into account boundary contribution
	n_boundary_op( r, latest_iter, index, s, l ); // r -= boundary_op*latest_iter
        vector_PRECISION_minus( D_phi, eta, r, s->block[index].start*l->num_lattice_site_var,
                                s->block[index].start*l->num_lattice_site_var+s->block_vector_size, l );
        if ( l->relax_fac != 1.0 ) {
          vector_PRECISION_scale( D_phi, D_phi, relax_factor, 0, s->block[index].start*l->num_lattice_site_var,
...
void sixteen_color_schwarz_PRECISION( vector_PRECISION *phi, vector_PRECISION *D_phi, vector_PRECISION *eta, const int cycles, int res, 
  ...
  if ( s->num_colors == 2 ) schwarz_PRECISION( phi, D_phi, eta, cycles, res, s, l, no_threading );
    ...
    if ( l->relax_fac != 1.0 )
      vector_PRECISION_scale( phi, x, relax_factor, 0, nb_thread_start*s->block_vector_size, nb_thread_end*s->block_vector_size, l );
    else
    ...
    vector_PRECISION_saxpy( &true_r, eta, &true_r, factor, 0, 1, 0, l->inner_vector_size, l );
    int nvecsf = nvec;
#ifdef HAVE_TM1p1
    if( ! (g.epsbar != 0 || g.epsbar_ig5_odd_shift != 0 || g.epsbar_ig5_odd_shift != 0))
      nvecsf *=  g.n_flavours;
#endif
    PRECISION r_norm[nvecsf], den[nvecsf];
    global_norm_PRECISION( r_norm, &true_r, 0, l->inner_vector_size, l, no_threading );
    global_norm_PRECISION( den, eta, 0, l->inner_vector_size, l, no_threading );
    VECTOR_LOOP( jj, nvecsf, jjj, r_norm[jj+jjj] /= den[jj+jjj];)
    char number[3]; sprintf( number, "%2d", 31+l->depth ); printf0("\033[1;%2sm|", number );
    for ( i=0; i<nvecsf; i++ )
      printf0(" ---- depth: %d, c: %d, schwarz iter %2d, vector %d, norm: %11.6le |", l->depth, s->num_colors, k, i, r_norm[i] );
    printf0("\033[0m\n"); fflush(0);
...
void schwarz_PRECISION_mvm_testfun( schwarz_PRECISION_struct *s, level_struct *l, struct Thread *threading ) {
  ...
  vector_PRECISION_define(&v1,0,0,v1.size,l);
  ...
  for ( i=0; i<nb; i++ ) {//printf0("sc test %d %d\n",s->block[i].start,s->num_block_sites);
    block_op( &v2, &v1, s->block[i].start*l->num_lattice_site_var, s, l, no_threading );
    boundary_op( &v2, &v1, i, s, l, no_threading );
  }
#ifdef HAVE_TM1p1
  //if (g.n_flavours==2)
  //for(int i=0;i<ivs*n_vect;i++)printf0("%d %d %d %13g %g %g\n",i/n_vect/l->num_lattice_site_var,(i/n_vect)%l->num_lattice_site_var,i%n_vect,creal_PRECISION(v2.vector_buffer[i]-v3.vector_buffer[i]),creal_PRECISION(v2.vector_buffer[i]),creal_PRECISION(v3.vector_buffer[i]));
#endif
  vector_PRECISION_minus( &v3, &v3, &v2, 0, l->inner_vector_size, l );
  global_norm_PRECISION( diff1, &v3, 0, l->inner_vector_size, l, no_threading );
  global_norm_PRECISION( diff2, &v2, 0, l->inner_vector_size, l, no_threading );

  for( i=0; i<n_vectsf; i++ )
