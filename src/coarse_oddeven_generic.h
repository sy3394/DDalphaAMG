 ...
 */
  ...
  void coarse_solve_odd_even_PRECISION_new( gmres_PRECISION_struct *p, operator_PRECISION_struct *op, level_struct *l, 
					    struct Thread *threading );
...
static inline void coarse_perform_fwd_bwd_subs_PRECISION_new( vector_PRECISION *x, vector_PRECISION *b, config_PRECISION A, int start, int end, level_struct *l ) {
  ...
  for ( s=start; s<end; s++ ) {  
    ...
    for ( i=n2-1; i>=0; i-- ) {
      for ( j=i+1; j<n2; j++ ) 
	VECTOR_LOOP(jj, nvec, jjj, x_pt[i*nvec_x+jj+jjj] -= A[i*n2+j]*x_pt[j*nvec_x+jj+jjj];)
	  VECTOR_LOOP( jj, nvec, jjj, x_pt[i*nvec_x+jj+jjj] /= A[i*(n2+1)];)
    }
    x_pt += n2*nvec_x;
    b_pt += n2*nvec_b;
    A += oo_inv_size;
